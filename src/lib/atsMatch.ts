...
export function computeAtsMatch(jobDescription: string, resumeText: string): number {
...
export function computePassedAts(
  pageCount: number,
  hasJobDescription: boolean,
  atsMatchPercent: number,
): boolean {
  return pageCount === 1 && (!hasJobDescription || atsMatchPercent >= 70);
}
